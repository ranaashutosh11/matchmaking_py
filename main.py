...
def match():
    ...
    if (request.method == 'POST'):
        ...
        if (sex is "Male" and personality is "Extrovert" and prefer is "Female"):
            ...
            mycursor.execute(
                "SELECT id FROM match ta LEFT JOIN found tb ON ta.ID = tb.ID2 WHERE sex = 'Female' and prefer = 'Male' and WHERE tb.Id IS NULL")
            matchid = mycursor.fetchall()
            ...
            mycursor.execute(
                "SELECT id FROM match ta LEFT JOIN found tb ON ta.ID = tb.ID2 WHERE sex = 'Male' and prefer = 'Male' and WHERE tb.Id IS NULL")
            matchid = mycursor.fetchall()
            ...
            mycursor.execute(
                "SELECT id FROM match ta LEFT JOIN found tb ON ta.ID = tb.ID2 WHERE sex = 'Male' and prefer = 'Female' and WHERE tb.Id IS NULL")
            matchid = mycursor.fetchall()
            ...
            mycursor.execute(
                "SELECT id FROM match ta LEFT JOIN found tb ON ta.ID = tb.ID2 WHERE sex = 'Female' and prefer = 'Female' and WHERE tb.Id IS NULL")
            matchid = mycursor.fetchall()
        if(matchid is None):
            ...
            idd2 = ' '.join([str(item) for item in matchid[0][0] ])
            entry = Found(id=id, id2=idd2)
            db.session.add(entry)
            db.session.commit()
            mycursor.execute("select email from registered where id='" + idd2 + "'")
            e = mycursor.fetchone()
            email = ' '.join([str(item) for item in e ])
            return render_template("match.html", msg=email)
    mydb.commit()
    mycursor.close()
